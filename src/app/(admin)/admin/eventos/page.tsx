  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-3xl text-foreground">Eventos</h1>
          <p className="text-muted text-sm mt-1">{events.length} eventos</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {events.map(ev => (
                <tr key={ev.id} className="border-b border-theme hover:bg-teal/[0.03]">
                  ...
                  <td className="px-4 py-3 font-semibold text-foreground max-w-[180px] truncate">{ev.title}</td>
                  ...
                  <td className="px-4 py-3 text-foreground font-semibold">{ev.price ? `${ev.price}€` : "Gratis"}</td>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? "Editar evento" : "Nuevo evento"} wide>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <label className="text-xs font-bold text-muted uppercase mb-1 block">Estado</label>
              <select className="input" value={form.status} onChange={e => set("status", e.target.value)}>
                {STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            <div>
              <label className="text-xs font-bold text-muted uppercase mb-1 block">Aforo</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-muted uppercase mb-1 block">Inicio</label>
              <input className="input" type="datetime-local" value={form.start_date} onChange={e => set("start_date", e.target.value)} />
            </div>
            <div>
              <label className="text-xs font-bold text-muted uppercase mb-1 block">Fin</label>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <label className="text-xs font-bold text-muted uppercase mb-1 block">Precio miembros</label>
            ...
            <div>
              <label className="text-xs font-bold text-muted uppercase mb-1 block">Ubicación</label>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-sm font-medium text-secondary">Evento online</span>
          </label>
          {form.is_online && (
            <div>
              <label className="text-xs font-bold text-muted uppercase mb-1 block">URL del evento online</label>
